...
func (s *MessageService) CreateMessage(ctx context.Context, userID, sessionID, role, content string) (model.Message, error) {
	...
	_ = cache.DeleteMessagePageCaches(ctx, sessionID)
...
func (s *MessageService) ListMessages(ctx context.Context, userID string, sessionID string) ([]model.Message, error) {
	...
	if err != nil {
		metrics.MessageCacheErrorTotal.Add(1)
	}
	if err == nil && hit {
		metrics.MessageCacheHitTotal.Add(1)
		return cachedMessages, nil
	}
	metrics.MessageCacheMissTotal.Add(1)
...
func (s *MessageService) ListMessagesPage(ctx context.Context, userID string, sessionID string, page int, pageSize int) ([]model.Message, int64, error) {
	...
	cachedMessages, cachedTotal, hit, err := cache.GetMessagePage(ctx, sessionID, page, pageSize)
	if err != nil {
		metrics.MessageCacheErrorTotal.Add(1)
	}
	if err == nil && hit {
		metrics.MessageCacheHitTotal.Add(1)
		return cachedMessages, cachedTotal, nil
	}
	metrics.MessageCacheMissTotal.Add(1)
	...
	_ = cache.SetMessagePage(ctx, sessionID, page, pageSize, messages, total)
