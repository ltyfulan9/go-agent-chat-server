...
type KnowledgeHandler struct {
	knowledgeService *service.KnowledgeService //负责知识库的增删查改
...
func (h *KnowledgeHandler) ChatWithKnowledge(ctx context.Context, c *app.RequestContext) {
	var req ChatWithKnowledgeRequest //接收请求体
	if err := c.BindAndValidate(&req); err != nil {
		c.JSON(consts.StatusBadRequest, Error(ecode.CodeInvalidRequest, "invalid request"))
		return
	} //检查session_id和message
	...
	userID := CurrentUserID(c) //获取当前用户ID
	...
	//把文档拼成额外的上下文，调用ChatWithExtraContext
...
func buildKnowledgeContext(docs []model.KnowledgeDoc) string { //把文档拼成prompt上下文
	if len(docs) == 0 {
		return "No matching knowledge documents were found."
	}

	parts := make([]string, 0, len(docs)) //创建parts切片，遍历每篇文章
	for _, doc := range docs {
		content := doc.Content
		runes := []rune(content) //按字符长度截断，避免中文乱码
		if len(runes) > 1000 {
