...
func (h *ChatHandler) ChatStream(ctx context.Context, c *app.RequestContext) {
	...
	c.SetBodyStream(reader, -1) //-1表示长度位置，把reader作为HTTP响应体

	metrics.SSEConnectionsTotal.Add(1) //累计建立过多少个SSE连接
	metrics.IncSSEActive()             //当前正在进行中的SSE连接数

	go func() {
		defer metrics.DecSSEActive()
		defer writer.Close()
...
func (h *ChatHandler) OpenAIChatCompletions(ctx context.Context, c *app.RequestContext) {
	...
	messages := make([]model.Message, 0, len(req.Messages)) //大小为0，容量为messages的长度
	for _, m := range req.Messages {                        //遍历请求里的每一条消息
	...
	metrics.SSEConnectionsTotal.Add(1)
...
func writeSSE(w io.Writer, event string, payload interface{}) error {
	...
	if event != "" {
		if _, err := fmt.Fprintf(w, "event: %s\n", event); err != nil {
			metrics.SSEWriteErrorTotal.Add(1)
			return err
		}
	}
