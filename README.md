...
- API + Worker split for async chat jobs: `POST /api/chat/async` + `GET /api/chat/jobs/:id`
...
```text
go-agent-chat-server/
├── cmd/server/main.go      # API service
├── cmd/worker/main.go      # async chat worker
├── internal/
│   ├── agent/              # Eino runner
...
This current version keeps the original sync/SSE APIs and adds a light API + Worker split:

```text
POST /api/chat/async
  -> create chat_jobs row in MySQL
  -> publish chat.task.created to RabbitMQ
  -> cmd/worker consumes task
  -> worker calls LLM with controlled concurrency
  -> update chat_jobs status and answer
  -> client polls GET /api/chat/jobs/:id
```

Run worker locally:

```bash
ASYNC_QUEUE_ENABLED=true go run ./cmd/worker
```

Or start all services including worker:

```bash
make docker-up
```

More details: `docs/API_WORKER_ARCHITECTURE.md`.
...
This project includes lightweight runtime observability and benchmark tooling for local performance validation.

### Runtime debug endpoints

Start the server with `DEBUG_ADDR=:6060`, then access:
...
- `message_cache_hit_total` / `message_cache_miss_total`: message page cache hits and misses.
- `ip_rate_limit_blocked_total`: blocked IP-level requests.
- `user_llm_rate_limit_blocked_total`: blocked user-level LLM requests.
- `llm_inflight`: current LLM requests in progress.
- `llm_acquire_wait_ms_total`: accumulated wait time for LLM concurrency tokens.
- `sse_connections_active`: current active SSE connections.
- `rabbitmq_publish_failed_total`: failed RabbitMQ publish attempts.
- `chat_job_created_total` / `chat_job_success_total` / `chat_job_failed_total`: async chat job lifecycle metrics.

### Benchmark scripts

Run normal API benchmarks:

```bash
TOKEN=your_token SESSION_ID=your_session_id N=1000 C=50 ./scripts/bench_api.sh
```

Run mock chat benchmarks without depending on Ollama/Coze generation speed:

```bash
TOKEN=your_token SESSION_ID=your_session_id N=100 C=10 ./scripts/bench_mock_chat.sh
```

The mock model is only used to validate backend request processing, message persistence, cache access, limiter overhead, SSE/non-SSE chain behavior, and event publishing. It does not represent real LLM generation throughput.

### One-command benchmark report

For local benchmark runs, temporarily loosen rate limits to avoid measuring only 429 responses:

```bash
export IP_RATE_LIMIT=10000
export USER_LLM_RATE_LIMIT=1000
export MAX_CONCURRENT_LLM=4
export DEBUG_ADDR=:6060
go run ./cmd/server
```
...
```bash
python3 scripts/benchmark_suite.py --n 300 --c 30 --chat-n 40 --chat-c 4
```

The script registers a benchmark user, creates a session, inserts test messages, runs concurrent benchmarks for session/message APIs and mock chat, then writes:

```text
docs/BENCHMARK_RESULT.md
```

The generated report includes QPS, p95 latency, error count, message page cache hit ratio, cache-related p95 reduction, mock chat throughput, and selected runtime metrics from `/debug/vars`.
